...
var drag = {
    ...
    init : function (className){
        ...
        $(document).mousemove(function(event){
            //判断拖拽标识是否为允许，否则不进行操作
            if(!drag.permitDrag) return true;
            //执行移动的操作
        ...
        $.each($('.' + drag.class_name + ' ul'), function(index, value){
            ...
            if(e.pageX > box_x && e.pageX < box_x-0+box_width && e.pageY > box_y && e.pageY < box_y-0+box_height){
                ...
                || $(value).offset().top !== drag.old_elm.parent().offset().top){
                    $(value).css('background-color', drag.bg_color_hover);
                }
            }else{
                //恢复容器原背景色
                $(value).css('background-color', drag.bg_color);
            }
    ...
    operate: function(param){
        // 业务操作
        // var stepId = drag.target.data('id');
        // var roleId = drag.new_elm.data('id');
        // console.log(stepId);
        // console.log(roleId);
    }
