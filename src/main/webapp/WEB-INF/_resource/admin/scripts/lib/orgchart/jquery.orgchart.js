...
(function(factory) {
    ...
    if (opts.exportButton && !$chartContainer.find('.oc-export-btn').length) {
      var $exportBtn = $('<button>', {
        ...
        'click': function() {
          ...
          var sourceChart = $('.chartsize').addClass('canvasContainer').find('.orgchart:visible').get(0);
          // return
          var flag = opts.direction === 'l2r' || opts.direction === 'r2l';
          html2canvas(sourceChart, {
            'width': flag ? sourceChart.clientHeight : sourceChart.clientWidth + 20,
  ...
  function showSiblings($node, direction) {
    ...
    $siblings.find('.node:visible').addClass('slide').removeClass('slide-left slide-right').eq(-1).one('transitionend', function() {
      ...
      if (isInAction($node)) {
        switchHorizontalArrow($node);
        $node.children('.topEdge').removeClass('icon-up').addClass('icon-down');
      }
    });
  }
  ...
  function switchVerticalArrow($arrow) {
    $arrow.toggleClass('icon-up').toggleClass('icon-down');
  }
    ...
    if (opts.toggleSiblingsResp && (typeof opts.ajaxURL === 'undefined' || $node.closest('.nodes').data('siblingsLoaded'))) {
      ...
      if ($prevSib.length) {
        if ($prevSib.is('.hidden')) {
          $node.children('.leftEdge').addClass('icon-left').removeClass('icon-right');
        } else {
          $node.children('.leftEdge').addClass('icon-right').removeClass('icon-left');
        }
      }
      var $nextSib = $node.closest('table').parent().next();
      if ($nextSib.length) {
        if ($nextSib.is('.hidden')) {
          $node.children('.rightEdge').addClass('icon-right').removeClass('icon-left');
        } else {
          $node.children('.rightEdge').addClass('icon-left').removeClass('icon-right');
        }
      }
    } else {
      var $sibs = $node.closest('table').parent().siblings();
      var sibsVisible = $sibs.length ? !$sibs.is('.hidden') : false;
      $node.children('.leftEdge').toggleClass('icon-right', sibsVisible).toggleClass('icon-left', !sibsVisible);
      $node.children('.rightEdge').toggleClass('icon-left', sibsVisible).toggleClass('icon-right', !sibsVisible);
    }
  }
  ...
  function createNode(nodeData, level, opts) {
    ...
    var title;
    if(nodeData[opts.nodeTitle]){
      title = '<div class="title" title="' + nodeData[opts.nodeTitle] + '">' + nodeData[opts.nodeTitle] + '</div>';
    }else{
      title = '';
    }
    var con;
    if(nodeData[opts.nodeContent]){
      con = '<div class="content" title="' + nodeData[opts.nodeContentStr] + '">' + nodeData[opts.nodeContent] + '</div>';
    }else{
      con = '<div class="content empty"></div>';
    }
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + ' data-type="' + nodeData[opts.nodeType] + '"' + ' data-count="' + nodeData[opts.childrenCount] + '"' + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + ' class="type' + nodeData[opts.nodeType] + '">')
    ...
    $nodeDiv.on('mouseenter mouseleave', function(event) {
      ...
      if (event.type === 'mouseenter') {
        if ($topEdge.length) {
          ...
          $topEdge.toggleClass('icon-up', !flag).toggleClass('icon-down', flag);
        }
        if ($bottomEdge.length) {
          flag = getNodeState($node, 'children').visible;
          $bottomEdge.toggleClass('icon-down', !flag).toggleClass('icon-up', flag);
        }
        if ($leftEdge.length) {
          switchHorizontalArrow($node);
        }
      } else {
        $node.children('.edge').removeClass('icon-up icon-down icon-right icon-left');
      }
    });
    ...
    if (opts.draggable) {
      ...
      .on('drop', function(event) {
        ...
        if(!($dragged.data('type') !== 'undefined' && $dropZone.data('type') != 4 && $dropZone.data('type') !== 'undefined' && $dragged.attr('id') != $dropZone.attr('id'))){
          return;
        }
        if($dragged.data('type') == 4 && $dropZone.data('type') == 1){
          return;
        }

        core.confirm('您确定要移动吗？', function(sure){
          if(sure){
            opts.dropPost($dragged, $dropZone)
